...
long random_walk(long key, long delta_range, long key_range) {
  ...
  key %= key_range;
  key = (key < 0 ? -key : key); // How could key ever be less than 0?
...
int main(int argc, char **argv) {
  if (argc <= 1) {
    ...
    fprintf(stderr,
            "Usage: %s 2 <NUM> <RANGE> (for performance test with tree "
            "containing <NUM> iterations with keys 0..<RANGE>-1)\n",
            argv[0]);
    fprintf(stderr, "Usage: %s 3 <NINS> <NITS> <KRANGE> <KDELTARANGE>\n",
            argv[0]);
    fprintf(stderr,
            "       (performance test with tree containing <NINS> insertions, "
            "queried <NITS> times, with keys 0..<RANGE>-1\n");
    fprintf(stderr, "        with keys generated by random walk with delta "
                    "uniformly chosen from +/- <KDELTARANGE>/2)\n");
    exit(1);
  }
  ...
  case '1': {
    ...
    for (i = 0; i < 100; ++i) {
      ...
      boundssplay_tree_insert(st, (boundssplay_tree_key)insert_key,
  ...
  case '2': {
    ...
    printf("Performing %d random insertions, random queries and random "
           "deletions with key range %d\n",
  ...
  case '3': {
    ...
    /* First build the tree by inserting <NINS> items, using keys generated by
     * random walk */
    ...
    printf("Queries: %ld microseconds, %f microseconds/query\n", query_time,
           ((double)query_time) / number_of_queries);
    lasttime = gettimemicroseconds();
